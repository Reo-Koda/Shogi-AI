...
class NNEngine(usi_server.USIEngine):
    ...
    def select_best_move(self, moves):
        ...
        return cshogi.move_to_usi(best_move), best_value
    ...
    def think(self):
        try:
            ...
            # alpha-beta 法
            move, value = self.select_best_move(moves)

            self.info(NN_model.output_target(value), [move])
...
if __name__ == "__main__":
    # エンジンの始動
    ckpt = '20260301-181406'
    engine_base = os.path.splitext(os.path.basename(sys.argv[0]))[0]
    engine = NNEngine(engine_base + ckpt, f"../../pram/ckpt-{ckpt}.pt") # 実行ファイルからの相対パス
    engine.loop()
